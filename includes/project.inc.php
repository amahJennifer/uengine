...
if (isset($_POST['create-project'])) {
    ...
    if (
        ...
        empty($projectDeadline)
    ) {
        header(
            'Location: ../project.php?error=emptyfields&projectTitle=' .
                $projectTitle .
                '&projectDeadline=' .
                $projectDeadline .
                '&projectDescription=' .
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            if ($resultCheck > 0) {
                header(
                    'Location: ../project.php?error=projectalreadyexists&projectDescription=' .
                        $projectDescription
                );
                exit();
            } else {
                $sql =
                    'INSERT INTO projects (project_title,project_description,project_deadline) VALUES (?,?,?);';
                $stmt = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    header('Location: ../project.php?error=sqlerror');
                    exit();
